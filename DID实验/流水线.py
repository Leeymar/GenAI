...
OUTPUT_PANEL_PATH = 'DID_Panel_Ready_2.csv'
...
def match_control_group(
    ...
    print("\n" + "=" * 60)
    print("Step 4b: 活跃度筛选 + PSM 倾向得分匹配")
    print("=" * 60)
    ...
    abs_caliper = caliper
    print(f"   Caliper（固定绝对值）= {abs_caliper:.4f}")

    # ← 插入位置：匹配前基线诊断
    t_base_pre = author_summary[author_summary['is_treated'] == 1]['baseline_rcr']
    c_base_pre = author_summary[author_summary['is_treated'] == 0]['baseline_rcr']
    gap_pre = t_base_pre.mean() - c_base_pre.mean()
    print(f"\n   📊 【匹配前】baseline_rcr 对比：")
    print(f"   实验组：人数={len(t_base_pre):,}  "
          f"均值={t_base_pre.mean():.4f}  "
          f"中位数={t_base_pre.median():.4f}")
    print(f"   对照组：人数={len(c_base_pre):,}  "
          f"均值={c_base_pre.mean():.4f}  "
          f"中位数={c_base_pre.median():.4f}")
    print(f"   均值差距（匹配前）：{gap_pre:.4f}")

    # ── ✅ KD-Tree 加速匹配（替换原来的 Python 循环）─────────
    ...
    # 每次查找 ratio × 20 个候选，足够应对 caliper 过滤 + 不放回的消耗
    k_neighbors = min(ratio * 20, len(control_df))

    nbrs = NearestNeighbors(n_neighbors=k_neighbors, algorithm='kd_tree')
    nbrs.fit(ctrl_ps_arr)
    distances, indices = nbrs.kneighbors(trt_ps_arr)   # 批量计算，无 Python 循环
    ...
    print(f"   未找到匹配的实验组作者 ：{unmatched_count:,} 人"
          f"（{unmatched_count / len(treated_df) * 100:.1f}%）")
    print(f"   成功匹配对照组         ：{len(matched_control_ids):,} 人")

    if unmatched_count > len(treated_df) * 0.1:
        print(f"\n   ⚠️  超过 10% 实验组未匹配，建议修改全局配置：")
        print(f"      PSM_CALIPER = 0.10  （放宽 caliper）")
        print(f"      或 MATCH_RATIO = 1  （1:1 精确匹配）")

    # ── 保留匹配后的面板 ──────────────────────────────────────
    treated_ids = set(treated_df['author_id'])
    matched_ids = treated_ids | set(matched_control_ids)
    ...
    print(f"\n   PSM 匹配后（1:{ratio}，caliper={caliper}）：")
    print(f"   实验组：{n_treat_after:,} 人  |  对照组：{n_ctrl_after:,} 人")

    # ── 匹配后基线诊断 ────────────────────────────────────────
    matched_summary = author_summary[author_summary['author_id'].isin(matched_ids)]
    t_base = matched_summary[matched_summary['is_treated'] == 1]['baseline_rcr']
    c_base = matched_summary[matched_summary['is_treated'] == 0]['baseline_rcr']
    ...
    if abs(gap) >= 0.05:
        print(f"\n   💡 调参建议：")
        print(f"      1. 修改 PSM_CALIPER = 0.02 + MATCH_RATIO = 1（精确优先）")
