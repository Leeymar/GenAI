...
# 1. 路径设置
# =========================
input_csv = "E:\PythonProject\GenAI\data_process\derived\genai_regression_enriched_v3_fixed_winsor1pct.csv"
output_excel = "three_tables_fixed_suowei.xlsx"
...
# =========================
# Reviewer robustness check:
# Alternative RCR transformation
# =========================
df["Log_RCR_001"] = np.log(df["RCR"] + 0.01)
...
def build_rcr_robustness_table():
    """
    Robustness test for reviewer comment:
    replace ln(RCR+1) with ln(RCR+0.01)
    """
    model_specs = []
    ...
    for add_topic in [False, True]:
        m, n, f = run_model(
            "Log_RCR_001",
            model_type="baseline",
            add_topic_fe=add_topic
        )

        model_specs.append({
            "col_name": "Log_RCR+0.01_" + ("2" if add_topic else "1"),
            "model": m,
            "nobs": n,
            "r2": m.rsquared,
            "year_fe": "Yes",
            "topic_fe": "Yes" if add_topic else "No",
            "formula": f
        })
...
rcr_robustness_table, rcr_robustness_specs = build_rcr_robustness_table()
...
with pd.ExcelWriter(output_excel, engine="openpyxl") as writer:
    ...
    rcr_robustness_table.to_excel(writer, sheet_name="RCR稳健性检验", index=False)
